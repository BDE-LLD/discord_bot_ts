...
class UserInfo {
    ...
    async userInfo(
        ...
        interaction: CommandInteraction
            ...
            if(user) {
                const coa: any[] = await client42.fetch("/users/" + login + "/coalitions?");
                ...
                const cursus = user.cursus_users.filter((cursus: any) => cursus.cursus_id === 21)[0];
                if(cursus) {
                    embed.addFields(
						{
							name: "level",
                ...
                if (user['alumni?']) embed.addFields({name: "alumni", value: "🟢", inline: true});
                if (user.groups.length) {
					let groups: any[] = [];
					user.groups.forEach((group: any) => groups.push(group.name.toLowerCase()));
					embed.addFields({name: "groups", value: groups.join("\n"), inline: true});
				}
				if (user.titles.length) {
					...
					embed.addFields({name: "titles", value: titles.join("\n"), inline: true});
				}
                if (user.location) embed.addFields({name: "Location", value: user.location, inline: true});
                interaction.editReply({content: "", embeds: [embed] });
            }
            else {
                interaction.editReply({content: `${login} is not found in the 42's intranet.`});
            }
        }
}
